...
public class FileUtil {
  ...
  public static final File APP_DATA_DIR =
      new File(App.Companion.getInstance().getExternalFilesDir(null).getAbsolutePath());
  ...
  public static String readFileFromUri(Uri uri) throws IOException {
    InputStream inputStream = App.Companion.getInstance().getContentResolver().openInputStream(uri);
    StringBuilder sb = new StringBuilder();
  ...
  public static void writeFileFromUri(Uri uri, String content) throws IOException {
    OutputStream outputStream = App.Companion.getInstance().getContentResolver().openOutputStream(uri);
