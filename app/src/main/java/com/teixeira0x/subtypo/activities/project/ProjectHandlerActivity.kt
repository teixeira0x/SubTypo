 ...
 *
 * @author Felipe teixeira0x
 */
abstract class ProjectHandlerActivity : com.teixeira0x.subtypo.activities.project.BaseProjectActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    project =
      intent?.extras?.getParcelableCompat<Project>(com.teixeira0x.subtypo.activities.project.BaseProjectActivity.Companion.KEY_PROJECT)
  ...
  private fun initializeProject() {
    ...
    coroutineScope.launch {
      ...
      try {
        subtitles = com.teixeira0x.subtypo.project.ProjectRepository.getProjectSubtitles(project)
      } catch (e: Exception) {
  ...
  protected fun saveProjectAsync() {
    ...
    coroutineScope.launch(Dispatchers.IO) {
      try {
        com.teixeira0x.subtypo.project.ProjectRepository.writeSubtitleDataFile(project.path, subtitlesViewModel.subtitles)
      } catch (e: Exception) {
        withContext(Dispatchers.Main) {
