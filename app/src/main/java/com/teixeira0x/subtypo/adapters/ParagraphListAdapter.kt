...
class ParagraphListAdapter(
  ...
  val onClickListener:
    (view: View, position: Int, paragraph: Paragraph) -> Unit,
  val onLongClickListener:
    (view: View, position: Int, paragraph: Paragraph) -> Boolean,
  ...
  override fun onCreateViewHolder(
    ...
    return ParagraphViewHolder(
      LayoutParagraphItemBinding.inflate(
  ...
  override fun onBindViewHolder(holder: ParagraphViewHolder, position: Int) {
    holder.binding.apply {
      ...
      startAndEnd.text =
        "${paragraph.startTime.formattedTime}|${paragraph.endTime.formattedTime}"

      inVideo.visibility =
        if (videoParagraphIndexer.contains(position)) View.VISIBLE
        else View.INVISIBLE
  ...
  fun setVideoPosition(videoPosition: Long) {
    ...
    paragraphs?.forEach {
      if (
        videoPosition >= it.startTime.milliseconds &&
          videoPosition <= it.endTime.milliseconds
      ) {
