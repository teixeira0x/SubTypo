...
class ParagraphValidator(val context: Context) {
  ...
  fun checkTime(formattedTime: String): com.teixeira0x.subtypo.data.ValidationResult {
    return if (TimeUtils.isValidTime(formattedTime.split(":"))) {
      com.teixeira0x.subtypo.data.ValidationResult.Success
    } else com.teixeira0x.subtypo.data.ValidationResult.Error(context.getString(R.string.subtitle_paragraph_invalid_time))
  }

  fun checkText(text: String): com.teixeira0x.subtypo.data.ValidationResult {
    return if (text.lines().firstOrNull { it.isEmpty() || it.isBlank() } == null) {
      com.teixeira0x.subtypo.data.ValidationResult.Success
    } else com.teixeira0x.subtypo.data.ValidationResult.Error(context.getString(R.string.warn_text_field_cannot_empty))
  }
