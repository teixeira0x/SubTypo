  ...
  private fun setTextWatchers() {
    binding.tieStartTime.afterTextChanged { s ->
      when (val startTimeResult = paragraphValidator.checkTime(s.toString().trim())) {
        is com.teixeira0x.subtypo.data.ValidationResult.Success -> binding.tilStartTime.isErrorEnabled = false
        is com.teixeira0x.subtypo.data.ValidationResult.Error -> binding.tilStartTime.setError(startTimeResult.message)
      }
      updateSaveButton()
    }
    binding.tieEndTime.afterTextChanged { s ->
      when (val endTimeResult = paragraphValidator.checkTime(s.toString().trim())) {
        is com.teixeira0x.subtypo.data.ValidationResult.Success -> binding.tilEndTime.isErrorEnabled = false
        is com.teixeira0x.subtypo.data.ValidationResult.Error -> binding.tilEndTime.setError(endTimeResult.message)
      }
      updateSaveButton()
    }
    binding.tieText.afterTextChanged { s ->
      when (val textResult = paragraphValidator.checkText(s.toString().trim())) {
        is com.teixeira0x.subtypo.data.ValidationResult.Success -> binding.tilText.isErrorEnabled = false
        is com.teixeira0x.subtypo.data.ValidationResult.Error -> binding.tilText.setError(textResult.message)
      }
