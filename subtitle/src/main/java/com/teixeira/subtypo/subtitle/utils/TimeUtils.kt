...
package com.teixeira.subtypo.subtitle.utils
...
object TimeUtils {
  ...
  fun String.getMilliseconds(): Long {
    ...
    require(isValidTime(timeParts)) { "Time format must be `hh:mm:ss,SSS`" }
  ...
  fun isInRange(value: String, min: Int, max: Int, expectedLength: Int): Boolean {
    return value.length == expectedLength && value.toIntOrNull()?.let { it in min..max } ?: false
  }
}
