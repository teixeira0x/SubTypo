...
class SubRipFormat : SubtitleFormat("SubRip", ".srt") {
   ...
   */
  private fun parseCaptionNumber(
    expectedNumber: Int,
    lineIndex: Int,
    line: String,
  ): Int {
    ...
    if (expectedNumber != number) {
      addError(
        SyntaxError(
          "Found number: $number, expected number: $expectedNumber",
          lineIndex,
        )
      )
    }
  ...
  /**
   * Parses the given line of timing code and returns the timings if it is valid
   * code.
   *
